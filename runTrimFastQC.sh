...
cd "/athena/angsd/scratch/few4007/angsd-files/project_2/fastqZipped"
...
for file in "$input_dir"/*_1.fastq; do
  ...
  if [ -f "$input_dir/${base}_1.fastq.gz" ] && [ -f "$input_dir/${base}_2.fastq.gz" ]; then
    # Create output directories for trimmed and fastqc results
    mkdir -p "$output_dir/$base/trimmed"
    ...
    trim_galore --paired \
      ...
      --illumina \
      -j 4 \
      --fastqc_args "--extract --outdir=$output_dir/fastqc_out/$base" \
