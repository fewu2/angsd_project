...
index_out="HS_index_2"

# Set working directory 
cd /athena/angsd/scratch/few4007/angsd-files/project_2/

# Check if genome and annotation files exist, else throw error
if [ ! -f "$assembly" ]; then
    echo "Specified assembly input is missing."
    exit 1
fi

if [ ! -f "$gtf" ]; then
    echo "Specified .GTF annotation file is missing."
    exit 1
fi
